    ...
    return (
      ...
      >
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            fetch("http://192.168.43.18:8000/forgot-password/", {
            method: "get",
            headers: {
            ...
            .then(response => {
              if (response.status === 200) {
                alert('We received your request ! We will reach you soon !')
              } 
            })
            .catch(err => {
              alert(err);
  ...
  render() {
    ...
    return (
      <KeyboardAwareScrollView animated={true}>
        ...
        >
          <Text style={{ color: "#fff", fontSize: 12 }}>{this.state.error}</Text>
          <Overlay
